defmodule Sagents.AgentTest do
  ...
  describe "new/1" do
    ...
    test "creates agent with tools" do
      ...
      # Now includes custom tool + write_todos (TodoList) + 7 filesystem tools (list_files, read_file, create_file, replace_file_text, find_in_file, delete_file, move_file) + SubAgents
  ...
  describe "middleware composition - default behavior" do
    ...
    test "combines user tools with middleware tools" do
      ...
      # user_tool + write_todos + 7 filesystem tools + tool1 + SubAgents = 11
      assert length(agent.tools) == 11
      tool_names = Enum.map(agent.tools, & &1.name)
      assert "write_todos" in tool_names
